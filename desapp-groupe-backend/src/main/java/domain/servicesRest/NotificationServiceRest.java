...
public class NotificationServiceRest {
    ...
    @GET
    @Path("systemNotificationsFor/{userId}")
    @Produces("application/json")
    public List<SystemNotification> systemNotificationsFor(@PathParam("userId") final Integer userId)
    {
        User user = userService.getUser(userId);
        return notificationService.systemNotificationsFor(user);
    }
